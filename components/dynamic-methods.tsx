...
export default function DynamicMethods() {
  ...
  async function fetchFlowBalance() {
    if (!primaryWallet || !isFlowWallet(primaryWallet)) return;
    try {
      setIsLoading(true);
      // Use our updated FlowService instead of Dynamic's getAccount() to avoid old Cadence syntax
      const address = primaryWallet.address;
      const balance = await FlowService.getBalance(address);
      setResult(safeStringify({ 
        address,
  ...
  async function signFlowMessage() {
    if (!primaryWallet || !isFlowWallet(primaryWallet)) return;
    try {
      setIsLoading(true);
      const result = await primaryWallet.signMessage("Hello from ShowUp!");
      setResult(safeStringify(result));
    } catch (error) {
      setResult(safeStringify({ error: error instanceof Error ? error.message : 'Unknown error occurred' }));
    } finally {
  ...
  <Button variant="outline" onClick={fetchEthereumMessage} className="cursor-pointer">
    Sign Message
            ...
            {primaryWallet && isFlowWallet(primaryWallet) && (
		<div className="pt-2">
      <div className="text-xs uppercase tracking-wide opacity-60 mb-2">
        Flow Wallet Methods
      </div>
  ...
  <Button variant="outline" onClick={fetchFlowBalance} className="cursor-pointer">
    Fetch Account & Balance
  </Button>

  <Button variant="outline" onClick={signFlowMessage} className="cursor-pointer">
    Sign Message
  </Button>
      </div>
    </div>
